"""
SGRADT 6.0 - Training Script
Versión mejorada con 10 features:
- Core: body, range
- Momentum: Stochastic (K, D), RSI
- Trend: ADX, +DI, -DI, MACD Histogram
- Volatility: ATR%
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='SGRADT 6.0 - Entrenamiento con 10 features'
    )
    ...
    # Stochastic parameters (SGRADT 6.0 defaults)
    ...
    # ADX parameters (SGRADT 6.0 defaults)
    ...
    # RSI parameters (NEW in SGRADT 6.0)
    parser.add_argument('--rsi_period', type=int, default=14,
                       help='RSI period')
    
    # MACD parameters (NEW in SGRADT 6.0)
    ...
    # ATR parameters (NEW in SGRADT 6.0)
    ...
    parser.add_argument('--strategy', type=str, 
                       ...
                       default='combined',
                       help='Estrategia de señales a usar')
    ...
    print(f"\n{'='*70}")
    print(f"SGRADT 6.0 - Training Strategy Model (10 Features)")
    print(f"{'='*70}")
    ...
    rsi_inst = RSIIndicator(close=df['close'], window=args.rsi_period)
    df['rsi'] = rsi_inst.rsi()
    
    print("Calculando MACD...")
    macd_inst = MACD(
        close=df['close'],
        window_slow=args.macd_slow,
        window_fast=args.macd_fast,
        window_sign=args.macd_signal
    )
    macd_main = macd_inst.macd()
    macd_signal = macd_inst.macd_signal()
    df['macd_hist'] = macd_main - macd_signal
    ...
    atr_inst = AverageTrueRange(
        ...
        low=df['low'], 
        close=df['close'], 
        window=args.atr_period
    )
    df['atr'] = atr_inst.average_true_range()
    df['atr_pct'] = (df['atr'] / df['close']) * 100
    
    # ========== CREAR FEATURES (ORDEN IMPORTANTE) ==========
    ...
    df['feat_atr_pct'] = df['atr_pct']
    
    # ORDEN DE FEATURES (CRÍTICO - debe coincidir con el EA)
    features_list = [
        'feat_body',           # 0
        'feat_range',          # 1
        'feat_stoch_main',     # 2
    ...
    print(f"\n{'='*70}")
    print(f"FEATURES CONFIGURADAS (10 features)")
    print(f"{'='*70}")
    ...
    output_path = output_dir / f"{csv_path.stem}_SGRADT60_{args.strategy}.onnx"
    ...
    meta = {
        ...
        # RSI config (NEW in 6.0)
        "rsi_period": args.rsi_period,
        
        # MACD config (NEW in 6.0)
        "macd_fast": args.macd_fast,
        "macd_slow": args.macd_slow,
        "macd_signal": args.macd_signal,
        
        # ATR config (NEW in 6.0)
    ...
    meta_path = output_dir / f"{csv_path.stem}_SGRADT60_{args.strategy}.meta.json"
    ...
    print(f"\n{'='*70}")
    print(f"RESUMEN FINAL - SGRADT 6.0")
    print(f"{'='*70}")
    ...
    print(f"    • RSI ({args.rsi_period})")
    print(f"    • MACD Histogram ({args.macd_fast},{args.macd_slow},{args.macd_signal})")
    print(f"    • ATR% ({args.atr_period})")
    print(f"{'='*70}\n")
