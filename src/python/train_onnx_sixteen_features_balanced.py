...
# --- CONFIGURATION ---
parser = argparse.ArgumentParser(description="Train ONNX model with BALANCED THREE-CLASS targets")
...
output_filename = os.path.join(
    output_dir,
    Path(csv_file).stem + f"_3class_w{window}_f{future}_atr{atr_period}_minp{min_profit_atr}_adx{adx_period}_adxm{adx_min}.onnx"
)
output_filename = str(output_filename).replace("_rates", "")

print(colorize("=" * 70, Colors.CYAN))
print(colorize("BALANCED 3-CLASS TRAINING (BUY / HOLD / SELL)", Colors.CYAN))
print(colorize("=" * 70, Colors.CYAN))
...
# ============================================================================
# 7. GENERATE BALANCED THREE-CLASS TARGET
# ============================================================================
print(colorize(f"[6/7] Generating BALANCED 3-class targets (future={future}, min_profit_atr={min_profit_atr})...", Colors.BLUE))
print(colorize("    0 = HOLD (neutral)", Colors.YELLOW))
print(colorize("    1 = BUY (bullish opportunity)", Colors.GREEN))
print(colorize("    2 = SELL (bearish opportunity)", Colors.RED))

labels = np.zeros(len(df), dtype=int)  # Default to HOLD
...
for i in range(len(df) - future):
    ...
    # Calculate potential profit (upside) and loss (downside) in ATR units
    max_upside   = (future_highs.max() - entry_price) / current_atr
    max_downside = (entry_price - future_lows.min()) / current_atr
    
    # Classification logic
    if max_upside >= min_profit_atr and max_upside > max_downside:
        # Strong bullish opportunity
        labels[i] = 1  # BUY
    elif max_downside >= min_profit_atr and max_downside > max_upside:
        # Strong bearish opportunity (profitable short)
        labels[i] = 2  # SELL
    # else: remains 0 (HOLD) - choppy/neutral conditions
...
print(f"{colorize('Class distribution:', Colors.WHITE)}")
print(f"  HOLD (0): {colorize(str(class_dist.get(0, 0)), Colors.YELLOW)} ({colorize(f'{(class_dist.get(0, 0)/len(y)*100):.2f}%', Colors.YELLOW)})")
print(f"  BUY  (1): {colorize(str(class_dist.get(1, 0)), Colors.GREEN)} ({colorize(f'{(class_dist.get(1, 0)/len(y)*100):.2f}%', Colors.GREEN)})")
print(f"  SELL (2): {colorize(str(class_dist.get(2, 0)), Colors.RED)} ({colorize(f'{(class_dist.get(2, 0)/len(y)*100):.2f}%', Colors.RED)})")
...
entry2 = onx.metadata_props.add()
entry2.key = "class_labels"
entry2.value = "0=HOLD,1=BUY,2=SELL"
...
print(colorize(f"✓ Classes:          3 (HOLD/BUY/SELL)", Colors.GREEN))
