//+------------------------------------------------------------------+
//|                                           EnsembleSellEA.mq5      |
//|  EA de Ensemble Sell-Only con 5 modelos ONNX                     |
...
input string InpModelA_Path = "model_A_impulse_sell.onnx";     // Modelo A (Impulse)
input string InpModelB_Path = "model_B_swing_sell.onnx";       // Modelo B (Swing)
input string InpModelC_Path = "model_C_trend_sell.onnx";       // Modelo C (Trend)
input string InpModelD_Path = "model_D_structure_sell.onnx";   // Modelo D (Structure)
input string InpModelE_Path = "model_E_volatility_sell.onnx";  // Modelo E (Volatility)
...
input double InpConfidenceThreshold = 0.60;  // Umbral confianza SELL (0-1)
...
int OnInit()
  {
   Log("=== EnsembleSellEA v1.10 Iniciando ===");
...
// Prepare the magic number
   ...
   Log("=== EnsembleSellEA Iniciado correctamente ===");
   return INIT_SUCCEEDED;
  }
...
// Triggers specifically when inputs are changed via the GUI
   ...
   Log("=== EnsembleSellEA Finalizado. Razon: " + IntegerToString(reason) + " ===");
  }
   ...
   if(InpVerbose && (g_logCounter >= InpLogEveryNBars || allOk))
     {
      ...
      string logMsg = StringFormat(
                         ...
                         ensembleSellProb, g_confidence_threshold, buyProb,
...
// Decision de trading
   ...
   string blockReason = "";
   if(ensembleSellProb < g_confidence_threshold)
      blockReason = StringFormat("SELL prob %.3f < umbral %.3f", ensembleSellProb, g_confidence_threshold);
   else
...
//+------------------------------------------------------------------+
bool RunInference(int modelIdx, const vectorf &inputVec, double &sellProbability)
  {
   ModelConfig m = g_models[modelIdx];  // <-- REFERENCIA, no copia
...
// outputProbs[0] = P(NoSell/Buy), outputProbs[1] = P(Sell) (modelo entrenado con label sell=1)
      ...
      case ENSEMBLE_MAJORITY:
        {
         ...
         return (totalVotes > 0) ? (double)sellVotes / totalVotes : 0.0;
        }
...
//| ABRIR POSICION SELL                                              |
//+------------------------------------------------------------------+
void OpenSellPosition(double ensembleProb, const double &modelProbs[])
  {
   ...
   if(!g_trade.Sell(InpLotSize, _Symbol, bid, sl, tp, comment))
     ...
     {
      Log("SELL ABIERTO | Bid: " + DoubleToString(bid, digits) + " | SL: " + DoubleToString(sl, digits) + " | TP: " + DoubleToString(tp, digits) + " | " + comment);
     }
  }
...
void SendInitialNotification()
  {
   string msg = "ENSEMBLE VOTER TRADING (SELL)\n\n";
