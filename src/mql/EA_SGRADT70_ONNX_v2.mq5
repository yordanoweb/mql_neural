//+------------------------------------------------------------------+
//|                                      EA_SGRADT70_ONNX_v2.mq5     |
//|                   SGRADT 7.0 v2 - NN-Driven Strategy (7 Features)|
//|                     Neural Network Makes ALL Decisions           |
//+------------------------------------------------------------------+
#property copyright "SGRADT 7.0 v2"
...
input string InpModelName = "USTEC_M5_SGRADT70_v2.onnx";  // Model filename
input string InpMetaFile  = "USTEC_M5_SGRADT70_v2.meta.json"; // Metadata file (optional)
input double InpMinConf   = 0.55;      // Minimum confidence (0.0-1.0)
input int    InpWindowSize = 20;       // Window size (must match training)
input int    InpFeaturesPerBar = 7;    // Features per bar (ALWAYS 7 for v2)
...
input int InpEMAPeriod = 9;            // EMA period (for ema_gate feature)
...
input int    InpMagic         = 7072;  // Magic number
...
//+------------------------------------------------------------------+
//| Run ONNX inference and execute trade if confidence met            |
//+------------------------------------------------------------------+
void RunInference()
  {
...
//--- Execute trade based on NN decision
      ...
      if(!HasPosition(POSITION_TYPE_BUY))
        {
         ...
         if(trade.Buy(InpLot, _Symbol, 0, sl, tp, 
                      StringFormat("SGRADT70_v2 BUY @%.1f%% | ATR=%.5f", max_prob*100, atr_value)))
           {
            Print("[BUY] Order opened | Confidence: ", DoubleToString(max_prob * 100, 2), 
                  "% | SL: ", DoubleToString(InpATRMultiplierSL, 1), "xATR | TP: ", 
                  DoubleToString(InpATRMultiplierTP, 1), "xATR");
            g_last_trade_bar = current_bar;
           }
        }
     }
   else if(predicted_class == 2)    // SELL signal from NN
     {
      if(!HasPosition(POSITION_TYPE_SELL))
        {
         ...
         if(trade.Sell(InpLot, _Symbol, 0, sl, tp, 
                       StringFormat("SGRADT70_v2 SELL @%.1f%% | ATR=%.5f", max_prob*100, atr_value)))
           {
...
//+------------------------------------------------------------------+
//| Prepare input data for ONNX model (7 features)                    |
//+------------------------------------------------------------------+
...
//--- Copy EMA data
   copied = CopyBuffer(g_ema_handle, 0, 0, window, ema_b);
   if(copied != window)
     {
      Print("[ERROR] CopyBuffer EMA failed: expected ", window, ", got ", copied);
      return false;
     }

//--- Copy Open prices
   copied = CopyOpen(_Symbol, _Period, 0, window, open_b);
   if(copied != window)
     {
...
//--- Copy Tick Volume (for volume_gate calculation)
   copied = CopyTickVolume(_Symbol, _Period, 0, window + 10, volume_b);
   if(copied != window + 10)
     {
      Print("[ERROR] CopyTickVolume failed: expected ", window + 10, ", got ", copied);
      return false;
     }

//--- Fill buffer with 7 features per bar
//--- Order: stoch_k, stoch_d, adx, pdi, mdi, ema_gate, volume_gate
   for(int i = 0; i < window; i++)
     {
      ...
      // Feature 6: EMA Gate (position relative to EMA)
      double ema_gate = 0.0;
      if(open_b[i] > ema_b[i])
...
//--- Indicators
   double ema[], adx[], di_plus[], di_minus[], stoch_k[], stoch_d[], atr[];
...
void PrintConfiguration()
  {
   ...
   Print("Features: 7 (stoch_k, stoch_d, adx, pdi, mdi, ema_gate, volume_gate)");
