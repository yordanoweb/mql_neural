//+------------------------------------------------------------------+
//|                                 MeanReversion_NDX100_M15.mq5     |
//+------------------------------------------------------------------+
#property strict

#include <Trade\Trade.mqh>

#resource "\\Files\\ndx100_rates_m15_meanrev.onnx" as uchar ExtModel[]
...
const int FEATURES    = 6;  // body, range, rsi, adx, k_slow, d_slow
...
void OnTick()
{
   ...
   int stoch_handle = iStochastic(_Symbol, _Period, 14, 3, 3, MODE_SMA, STO_CLOSECLOSE);
   double k_slow_buffer[], d_slow_buffer[];
   ArraySetAsSeries(k_slow_buffer, true);
   ArraySetAsSeries(d_slow_buffer, true);
   CopyBuffer(stoch_handle, 0, 0, WINDOW_SIZE, k_slow_buffer); // %K slow
   CopyBuffer(stoch_handle, 1, 0, WINDOW_SIZE, d_slow_buffer); // %D slow
   ...
   for(int i=0; i < WINDOW_SIZE; i++)
   {
      ...
      input_buffer[i * FEATURES + 4] = (float)(k_slow_buffer[mql_idx]); // %K slow
      input_buffer[i * FEATURES + 5] = (float)(d_slow_buffer[mql_idx]); // %D slow
   }
