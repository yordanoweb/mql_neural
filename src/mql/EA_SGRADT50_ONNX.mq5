...
//--- Initialize indicators
   ...
   Print("Indicators created: ADX(", InpADXPeriod, ") + Stochastic(",
         InpStochK, ",", InpStochD, ",", InpStochSlowing, ") + ATR(", InpATRPeriod, ") + EMA(", InpEMAPeriod, ")");
...
void UpdateIndicatorGlobals()
  {
   ...
   if(CopyBuffer(g_adx_handle, 0, 0, 1, adx_v) > 0)
      g_curr_adx = adx_v[0];
   if(CopyBuffer(g_adx_handle, 1, 0, 1, pdi_v) > 0)
      g_curr_pdi = pdi_v[0];
   if(CopyBuffer(g_adx_handle, 2, 0, 1, mdi_v) > 0)
      g_curr_mdi = mdi_v[0];

   double sk[1], sd[1];
   if(CopyBuffer(g_stoch_handle, 0, 0, 1, sk) > 0)
      g_stoch_k = sk[0];
   if(CopyBuffer(g_stoch_handle, 1, 0, 1, sd) > 0)
      g_stoch_d = sd[0];
  }
...
// Usamos el nivel de confianza de HOLD si la predicción es 0
   ...
   PrintFormat("\n%s | Inference #%d: %s signal | Conf: %.2f%% | Time: %s | Bar: %s | Position: %s",
               _Symbol,
               g_infer_count,
               signal_name,
               display_conf * 100,
