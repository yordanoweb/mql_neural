...
#resource "\\Files\\ndx100_rates_m5_SGRADT70.onnx" as uchar ExtModel[];
...
//--- Load ONNX model
   g_onnx_handle = OnnxCreateFromBuffer(ExtModel, ONNX_DEFAULT);

   if(g_onnx_handle == INVALID_HANDLE)
     {
      Print("[ERROR] Cannot load ONNX model");
      Print("Terminal build: ", TerminalInfoInteger(TERMINAL_BUILD));
      Print("Is tester: ", MQLInfoInteger(MQL_TESTER));
      return INIT_FAILED;
     }
   Print("ONNX handle: ", g_onnx_handle);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(bars_elapsed < InpForecastHorizon)
         continue;
