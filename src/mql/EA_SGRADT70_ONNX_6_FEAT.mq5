...
input double InpATRMultTP  = 3.0;   // ATR Multiplier for Take Profit (0=disabled)
...
int OnInit()
  {
   Print("\n", StringRepeat("-", 70));
   Print("    SGRADT 7.0 - EMA 9 STRATEGY (6 Features with DI+/DI-)");
   ...
   while(attempts < max_attempts)
     {
      ...
      int atr_count = CopyBuffer(g_atr_handle, 0, 0, required_bars, test_buf);  // NEW

      if(ema_count == required_bars &&
         adx_count == required_bars &&
         stoch_count == required_bars &&
         atr_count == required_bars)  // NEW
        {
...
void OnDeinit(const int reason)
  {
   ...
   if(g_atr_handle != INVALID_HANDLE)  // NEW
      IndicatorRelease(g_atr_handle);  // NEW
...
//--- Check confidence
   ...
   // NEW: Get current ATR value for SL/TP calculation
...
//--- Indicators
   double ema[], adx[], stoch_k[], stoch_d[], atr[];
   ...
   CopyBuffer(g_atr_handle, 0, 0, 1, atr);  // NEW
...
//--- Risk Settings
   ...
   // MODIFIED: Show ATR info instead of fixed points
