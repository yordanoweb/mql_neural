...
void OpenBuyPosition(double ensembleProb, const double &modelProbs[])
  {
   ...
   double sl = (InpSL_ATR_Mult > 0) ? NormalizeDouble(ask - InpSL_ATR_Mult * atr, digits) : 0.0;
   double tp = (InpTP_ATR_Mult > 0) ? NormalizeDouble(ask + InpTP_ATR_Mult * atr, digits) : 0.0;
   ...
   if(!g_trade.Buy(TradeSizeOptimized(), _Symbol, ask, sl, tp, comment))
     {
