#!/usr/bin/env python3
"""
train_ensemble_sell_only.py
=================
Genera un ensemble de 5 modelos ONNX sell-only con validación OOS cronológica.
Basado en train_ensemble_buy_only.py, con el target invertido (senal SELL).

Uso básico (genera los 5 modelos de una vez):
    python train_ensemble_sell_only.py --input-csv EURUSD_M5.csv --output-dir ./models --forward 10

Uso para un solo modelo:
    python train_ensemble_sell_only.py --input-csv EURUSD_M5.csv --output-dir ./models \
        --model-id B --window 15 --feature-set standard --forward 10

El script produce:
    - model_A_impulse_sell_SYMBOL.onnx
    - model_B_swing_sell_SYMBOL.onnx
    - model_C_trend_sell_SYMBOL.onnx
    - model_D_structure_sell_SYMBOL.onnx
    - model_E_volatility_sell_SYMBOL.onnx
    - ensemble_report_sell.json
    - ensemble_threshold_report_sell.json (sweep OOS 0.50..0.80)
    - ensemble_thresholds_sell.csv (mismo sweep en formato tabular)
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train sell-only RandomForest ensemble with OOS validation and export ONNX.",
    ...
    parser.add_argument("--forward", type=int, default=10,
                        help="Forward bars for sell label: close[t+forward] < close[t]")
...
def create_windows(df, features, window, forward, close_col, low_col, dec_percent = 0.5):
    """
    Crea ventanas deslizantes y target (SELL: precio baja al menos dec_percent%).
    Target SELL = 1 si el precio (low) alcanza la baja deseada en CUALQUIER
    vela dentro de las proximas `forward` velas, no solo en la vela final.
    Devuelve (X, y, indices).
    """
    df = df.copy()
    future_low = df[low_col].shift(-1)
    future_min = future_low[::-1].rolling(window=forward, min_periods=1).min()[::-1]
    df['target'] = (future_min < df[close_col] * (1 - dec_percent / 100)).astype(int)
    df.loc[future_min.isna(), 'target'] = np.nan
...
def evaluate_model(model, X, y):
    ...
    Devuelve dict con metricas (clase positiva = SELL).
    """
    ...
    # Confidence stats for SELL predictions
    sell_mask = y_pred == 1
    avg_confidence_sell = float(y_proba[sell_mask, 1].mean()) if sell_mask.any() else 0.0
    max_confidence_sell = float(y_proba[sell_mask, 1].max()) if sell_mask.any() else 0.0
    min_confidence_sell = float(y_proba[sell_mask, 1].min()) if sell_mask.any() else 0.0

    return {
        "balanced_accuracy": float(ba) if ba is not None else None,
        "precision_sell": float(prec) if prec is not None else None,
        "recall_sell": float(rec) if rec is not None else None,
        "roc_auc": float(roc) if roc is not None else None,
        "confusion_matrix": cm,
        "signals_sell_count": signal_count,
        "signals_sell_pct": round(100.0 * signal_count / total, 2) if total > 0 else 0.0,
        "avg_confidence_sell": round(avg_confidence_sell, 6),
        "max_confidence_sell": round(max_confidence_sell, 6),
        "min_confidence_sell": round(min_confidence_sell, 6),
        "total_samples": total,
...
def analyze_ensemble_thresholds(results, start=0.50, stop=0.80, step=0.01):
    ...
    La probabilidad SELL del ensemble es la media de las probabilidades SELL
    ...
    valid = [
        ...
        and r.get("oos_proba_sell") is not None
    ]
    ...
    for r in valid:
        maps.append({
            ...
            for idx, y, p in zip(
                r["oos_indices"], r["oos_y"], r["oos_proba_sell"]
            )
        })
    ...
    # Media simple de probabilidades SELL.
    ...
    while t <= float(stop) + 1e-9:
        ...
        thresholds.append({
            "threshold": round(t, 2),
            "precision_sell": round(float(precision), 6),
            "recall_sell": round(float(recall), 6),
            "balanced_accuracy": round(float(ba), 6),
            "signals_sell_count": signal_count,
            "signals_sell_pct": round(100.0 * signal_count / total, 2),
            "avg_confidence_sell": round(
    ...
    return {
        "status": "OK",
        "aggregation": "simple_mean_probability_sell",
...
def train_single_model(
    ...
    # Crear ventanas
    X, y, indices = create_windows(df, features, config['window'], forward, close_col, low_col)
    ...
    print(f"TRAIN  — BA: {train_metrics['balanced_accuracy']:.4f} | "
          f"Precision: {train_metrics['precision_sell']:.4f} | "
          f"Recall: {train_metrics['recall_sell']:.4f}")

    # Evaluacion OOS TEST (la importante)
    test_metrics = evaluate_model(model, X_test, y_test)
    print(f"TEST   — BA: {test_metrics['balanced_accuracy']:.4f} | "
          f"Precision: {test_metrics['precision_sell']:.4f} | "
          f"Recall: {test_metrics['recall_sell']:.4f} | "
          f"ROC-AUC: {test_metrics['roc_auc']:.4f}")
    print(f"TEST signals SELL: {test_metrics['signals_sell_count']} "
          f"({test_metrics['signals_sell_pct']}%)")
    ...
    metadata = {
        ...
        "training.target": f"min(low[t+1..t+{forward}]) < close[t] * 0.995",
        ...
        "validation.train_precision_sell": train_metrics['precision_sell'],
        "validation.train_recall_sell": train_metrics['recall_sell'],
        "validation.test_balanced_accuracy": test_metrics['balanced_accuracy'],
        "validation.test_precision_sell": test_metrics['precision_sell'],
        "validation.test_recall_sell": test_metrics['recall_sell'],
        "validation.test_roc_auc": test_metrics['roc_auc'],
        "validation.test_signals_sell_count": test_metrics['signals_sell_count'],
        "validation.test_signals_sell_pct": test_metrics['signals_sell_pct'],
        "validation.test_avg_confidence_sell": test_metrics['avg_confidence_sell'],
    ...
    return {
        ...
        "oos_y": y_test.tolist(),
        "oos_proba_sell": model.predict_proba(X_test)[:, 1].astype(float).tolist(),
    }
...
def main():
    ...
    print("=" * 70)
    print("  ENSEMBLE SELL-ONLY TRAINING")
    ...
    for mid in model_ids:
        ...
        output_filename = f"{args.output_prefix}_{mid}_{config['alias']}_sell_{resolved_symbol}.onnx"
    ...
    if len(results) > 1 or (len(results) == 1 and "error" not in results[0]):
        ...
        for r in results:
            if "error" in r:
                ...
                report["models"].append({
                    ...
                    "test_precision_sell": r["test_metrics"]["precision_sell"],
                    "test_recall_sell": r["test_metrics"]["recall_sell"],
                    "test_roc_auc": r["test_metrics"]["roc_auc"],
                    "test_signals_sell_count": r["test_metrics"]["signals_sell_count"],
                    "test_signals_sell_pct": r["test_metrics"]["signals_sell_pct"],
                    "test_avg_confidence_sell": r["test_metrics"]["avg_confidence_sell"],
                })

        report_path = os.path.join(args.output_dir, "ensemble_report_sell.json")
        ...
        # de las probabilidades SELL de los modelos.
        ensemble_analysis = analyze_ensemble_thresholds(results)

        threshold_report_path = os.path.join(
            args.output_dir, "ensemble_threshold_report_sell.json"
        )
        ...
        if ensemble_analysis.get("status") == "OK":
            ...
            threshold_csv_path = os.path.join(
                args.output_dir, "ensemble_thresholds_sell.csv"
            )
            ...
            for row in threshold_rows:
                print(
                    ...
                    f"{row['precision_sell']:<8.3f} "
                    f"{row['recall_sell']:<8.3f} "
                    f"{row['balanced_accuracy']:<8.3f} "
                    f"{row['signals_sell_count']:<9} "
                    f"{row['signals_sell_pct']:<7.2f} "
                    f"{row['avg_confidence_sell']:<9.3f}"
                )
        else:
            print(
        ...
        for m in report["models"]:
            if m.get("status") == "OK":
                print(f"  {m['model_id']:<4} {m['alias']:<12} {m['window']:<7} "
                      ...
                      f"{m['test_precision_sell']:.3f}   "
                      f"{m['test_recall_sell']:.3f}   "
                      f"{m['test_roc_auc']:.3f}   "
                      f"{m['test_signals_sell_count']}")
            else:
