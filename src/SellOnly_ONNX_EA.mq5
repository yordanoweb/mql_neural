...
input bool    InpMirrorEntryOperation = false; // Execute the opposite side when a sell signal is detected
...
void UpdateComment()
  {
   ...
   Comment("\n\n\nAI SELL-ONLY ", GetTimeframeString(_Period),
...
ENUM_ORDER_TYPE GetEntryOrderType()
  {
   return (g_runtime_mirror_entry_operation ? ORDER_TYPE_BUY : ORDER_TYPE_SELL);
  }
...
bool PerformInference()
  {
   ...
   Print("Inference Result: Prediction = ", g_prediction,
         ", Sell Confidence = ", DoubleToString(g_confidence*100, 2), "% / ",
   ...
   if(entry_allowed)
     {
      ...
      bool trade_ok = SendEntryWithManagedStops(m_trade,
                                                lot,
                                                entry_order_type,
                                                price,
                                                sl,
                                                tp,
                                                (is_buy_order ? "AI BUY@" : "AI SELL@") + DoubleToString(g_confidence, 2),
                                                InpUseSL);
      if(trade_ok)
